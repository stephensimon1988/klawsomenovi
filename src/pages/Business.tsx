      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div ref={tabsSectionRef}>
          <AnimatePresence>
            {showStickyTabs && (
              <motion.div
                ...
                className="fixed top-16 left-0 right-0 z-40 bg-background/95 backdrop-blur-md shadow-lg px-4 py-3 text-center border-b border-border"
              >
          ...
          <TabsContent value="hosted" className="mt-0">
            <div className="py-28 px-6 lg:px-12 bg-background text-center">
              <div className="container mx-auto max-w-4xl relative">
                ...
                <div className="inline-flex items-center gap-2 bg-secondary border border-border text-foreground font-bold text-sm px-5 py-2.5 rounded-full mb-7">
                  <MapPin className="w-4 h-4" /> Available within 50 miles of Novi, MI (48375) only
                </div>
                <p className="text-muted-foreground font-body text-base md:text-lg max-w-xl mx-auto mb-8 leading-relaxed">
                  {hosted?.subtitle}
                </p>
            ...
            <div className="py-28 px-6 lg:px-12 bg-secondary/50">
              <div className="container mx-auto max-w-4xl">
                <div className="rounded-2xl border border-border bg-background p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
                  <div>
                    <h3 className="font-heading text-xl md:text-2xl font-bold text-foreground mb-2">You earn 10% of every token played.</h3>
                ...
                <div className="flex flex-wrap gap-3 mb-8">
                  {venues.map((v) => (
                    <span key={v} className="bg-background border border-border rounded-full px-5 py-2.5 font-heading font-bold text-sm text-foreground/80 hover:bg-primary/5 hover:border-primary/40 transition-all cursor-default">
                      {v}
                    </span>
                  ))}
                </div>
              ...
              <div className="container mx-auto max-w-4xl">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-5 mb-8">
                  {partnerIncludes.map((item, index) => (
                    <motion.div
                      key={item.title}
                      className="rounded-2xl border border-border bg-background p-6 text-center hover:shadow-lg transition-shadow"
          ...
          <TabsContent value="plushie" className="mt-0">
            ...
            <div className="py-28 px-6 lg:px-12 bg-secondary/50">
              <div className="container mx-auto max-w-4xl">
                <div className="border-l-4 border-primary rounded-r-2xl bg-background p-5 flex items-center gap-4 mb-10 border border-border">
                ...
                <div className="grid md:grid-cols-3 gap-5 mb-10">
                  {pricingTiers.map((tier, index) => (
                    <motion.div
                      ...
                      className={`rounded-2xl p-8 text-center border ${
                        index === pricingTiers.length - 1
                          ? 'bg-klawsome-navy border-primary/30 text-white'
                    ...
                    >
                      <p className={`text-xs font-bold tracking-widest uppercase mb-2 ${index === pricingTiers.length - 1 ? 'text-primary' : 'text-primary'}`}>{tier.features?.[0] || ''}</p>
                  ...
                  <div className="grid md:grid-cols-3 gap-6">
                    {(plushie?.bullet_points || []).map((step, i) => (
                      <div key={i} className="text-center">
                        ...
                        <h6 className="font-heading text-base font-bold text-foreground mb-1">{step}</h6>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-7">
            {howSteps.map((step, index) => (
              <motion.div
                ...
                transition={{ duration: 0.5, delay: index * 0.15 }}
              >
                <div className="w-14 h-14 bg-primary text-white font-heading text-xl font-bold rounded-full flex items-center justify-center mx-auto mb-4">
                  {step.icon}
                </div>
        ...
        <div className="container mx-auto max-w-2xl">
          ...
          {submitted ? (
            <motion.div
              ...
              className="rounded-2xl border border-border bg-secondary/50 p-6 text-foreground font-heading font-bold text-lg"
            >
            ...
            <form onSubmit={handleSubmit} className="text-left">
              <div className="grid md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-heading font-bold text-foreground">Your Name</label>
                  <input type="text" required placeholder="Jane Smith" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="border border-border rounded-2xl px-4 py-3 font-body text-sm text-foreground bg-background placeholder:text-muted-foreground focus:border-primary outline-none transition-colors" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-heading font-bold text-foreground">Email Address</label>
                ...
                <div className="flex flex-col gap-2 md:col-span-2">
                  <label className="text-sm font-heading font-bold text-foreground">I'm interested in...</label>
                  <select value={formData.opportunity} onChange={(e) => setFormData({ ...formData, opportunity: e.target.value })} className="border border-border rounded-2xl px-4 py-3 font-body text-sm text-foreground bg-background focus:border-primary outline-none transition-colors">
                ...
                <div className="flex flex-col gap-2 md:col-span-2">
                  <label className="text-sm font-heading font-bold text-foreground">I am a...</label>
                  <select value={formData.business_type} onChange={(e) => setFormData({ ...formData, business_type: e.target.value })} className="border border-border rounded-2xl px-4 py-3 font-body text-sm text-foreground bg-background focus:border-primary outline-none transition-colors">
                    <option value="">Select...</option>
                    <option>Restaurant owner</option>
                    <option>Bubble Tea / Cafe owner</option>
                ...
                <div className="flex flex-col gap-2 md:col-span-2">
                  <label className="text-sm font-heading font-bold text-foreground">Tell us more</label>
                  <textarea placeholder="Share your idea, location, concept -- anything that helps us understand what you're looking for..." value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} rows={5} className="border border-border rounded-2xl px-4 py-3 font-body text-sm text-foreground bg-background placeholder:text-muted-foreground focus:border-primary outline-none transition-colors resize-y" />
                </div>
              </div>
              <div className="text-center mt-7">
                <Button type="submit" size="lg" className="rounded-full px-14 py-6 text-lg font-heading font-bold bg-primary hover:bg-primary/90 text-white">
                  Send Message →
                </Button>
