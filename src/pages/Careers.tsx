  ...
  <section className="relative min-h-[70vh] flex items-end overflow-hidden">
    ...
    <div className="relative z-10 container mx-auto px-6 lg:px-12 pb-20 pt-32">
      <div className="max-w-2xl">
        <h1 className="text-5xl md:text-7xl lg:text-8xl font-heading font-bold text-white uppercase leading-[0.95] mb-6">CAREERS</h1>
        <p className="text-white/70 font-body text-lg max-w-xl">Join the Klawsome team and help create a fun, welcoming experience for everyone!</p>
        <img src="https://images.squarespace-cdn.com/content/679927505e618d391ae386e6/8c57b6f1-0c08-4d2f-bb04-e54051ae7f0b/Klawsome_Fox__CUT.png?content-type=image%2Fpng" alt="Klawsome Fox" className="w-28 mt-8" loading="lazy" />
      </div>
    </div>
  </section>
);
...
const JobCard = ({ job }: { job: JobListing }) => (
  <div className="rounded-2xl border border-border bg-background overflow-hidden md:flex hover:shadow-lg transition-shadow">
...
const CareersInStore = () => {
  const { data: allJobs } = useCmsTable<JobListing>('job_listings');
  const jobs = allJobs?.filter(j => j.category === 'in-store') || [];
  ...
  return (
    <>
      ...
      <div className="space-y-8">{jobs.map(job => <JobCard key={job.id} job={job} />)}</div>
    </>
  );
};
...
const CareersHybrid = () => {
  const { data: allJobs } = useCmsTable<JobListing>('job_listings');
  const jobs = allJobs?.filter(j => j.category === 'hybrid') || [];
  if (jobs.length === 0) return null;
  return (
    <>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {jobs.map(job => (
          <div key={job.id} className="rounded-2xl border border-border bg-background p-6 text-center hover:shadow-lg transition-shadow">
            <img src="https://images.squarespace-cdn.com/content/v1/679927505e618d391ae386e6/8184018c-90f9-4412-a17e-7bdfbbdd62a9/klaw-penguin.png" alt="" className="w-20 mx-auto mb-4" loading="lazy" />
            <h3 className="font-heading font-bold text-lg text-foreground mb-4">{job.title}</h3>
            <div className="flex flex-col gap-2">
              {job.job_desc_url && <Button asChild size="sm" className="rounded-full font-heading font-bold bg-secondary text-foreground hover:bg-secondary/80"><a href={job.job_desc_url} target="_blank" rel="noopener noreferrer">Job Description</a></Button>}
              {job.apply_url && <Button asChild size="sm" className="rounded-full font-heading font-bold bg-primary hover:bg-primary/90 text-white"><a href={job.apply_url} target="_blank" rel="noopener noreferrer">Apply Here</a></Button>}
            </div>
          </div>
        ))}
      </div>
...
const CareersUnpaid = () => {
  const { data: allJobs } = useCmsTable<JobListing>('job_listings');
  const jobs = allJobs?.filter(j => j.category === 'unpaid') || [];
  if (jobs.length === 0) return null;
  return (
    <>
...
const SECTION_MAP: Record<string, React.ComponentType> = {
  hero: CareersHero,
  instore: CareersInStore,
  hybrid: CareersHybrid,
  unpaid: CareersUnpaid,
};

const FALLBACK_SECTIONS: PageSection[] = [
  { id: 'f1', page: 'careers', section_key: 'hero', label: 'Hero', sort_order: 1, is_visible: true, section_height: '70vh', wrapper_max_width: 'full', padding_y: '0', bg_color: '', bg_image_url: '', custom_css_class: '', columns: 1 },
  { id: 'f2', page: 'careers', section_key: 'instore', label: 'In-Store', sort_order: 2, is_visible: true, section_height: 'auto', wrapper_max_width: '1200px', padding_y: '7rem', bg_color: '', bg_image_url: '', custom_css_class: '', columns: 1 },
  { id: 'f3', page: 'careers', section_key: 'hybrid', label: 'Hybrid', sort_order: 3, is_visible: true, section_height: 'auto', wrapper_max_width: '1200px', padding_y: '7rem', bg_color: '', bg_image_url: '', custom_css_class: 'bg-secondary/50', columns: 1 },
  { id: 'f4', page: 'careers', section_key: 'unpaid', label: 'Unpaid', sort_order: 4, is_visible: true, section_height: 'auto', wrapper_max_width: '1200px', padding_y: '7rem', bg_color: '', bg_image_url: '', custom_css_class: '', columns: 1 },
];
